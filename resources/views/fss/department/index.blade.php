...
<form method="GET" action="{{ route('fss.departments.index') }}">
    <label for="search">
        ...
        <tbody id="department-list">
            @foreach($departments as $dept)
                <tr class="border-b text-sm">
                    ...
                    <td class="py-2">{{ $dept->users_count }}</td>
                    <td class="py-2 text-right">
                        <a href="{{ route('fss.departments.show', ["id" => $dept->id]) }}" class="text-blue-500">Detail</a>
                    </td>
                </tr>
            @endforeach
        </tbody>
